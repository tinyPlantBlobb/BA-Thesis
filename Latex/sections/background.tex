...
\subsection{Dropout}
Monte-Carlo dropout is the process of masking neurons in a Deep Neural Network randomly, based on a Bayesian probability to 0. This is usually done during training to reduce the chance of the model overfitting on the trainings data\cite{JMLR:v15:srivastava14a} \\
-> usually/commonly used in training \\
...
\subsection{Softmax}
maps a probability distribution to be between 0 and 1 by setting all values below 0 to 0 and scaleing averything that is about 0 to fall between 0 and 1 do that that sum of all the values is 1. 
