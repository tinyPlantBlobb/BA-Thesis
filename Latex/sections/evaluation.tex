...
for a unified score the baselines, which are the tranlsation and transcription probability, are multiplied, or rather multiplied since the transcription metrics estimate the wer quite well. 

the resulting pearson score that can be found in \ref{results} is calculated by correlating the product of translation probability and transcription probability with the the comet score divided\todo{figure out how to deal with 0 wer, aka perferct transcript} by the WER, which was choosen since the transcription probability is anti-correlated to the wer. 
...
\label{results}
\end{table}
other interesting values: the pearson correlation of the entropies in dropout shows a signigicant prediction value with a correlation of the mean of -0.23241 and a correlation of -0.17032 of the variance of the entropies on seamless.